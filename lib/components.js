...
class Textbox extends Component {
	...
	getKeyboardType() {
		...
		if (t.Nil.is(keyboardType) && this.typeInfo.innerType === t.Phone) {
			return "numeric";
		}
		return keyboardType;
	}
	...
	getLocals() {
		...
		locals.mask = this.props.options.mask;
	...
	getLocals() {
		...
		locals.label =
			this.props.ctx.auto !== "none"
				? locals.label.toUpperCase() || this.getDefaultLabel()
				: null;
...
class Select extends Component {
	...
	getOptions() {
		...
		/**Esse if esta comentado para remover a nullOption (valor padrao quando nao estiver selecionado nada)**/
...
class Struct extends Component {
	...
	onChange(fieldName, fieldValue, path) {
		...
		if (path[0] == "birth") {
			...
			maskDate = value.birth + "";
			maskDate = maskDate.replace(/^(\d\d)(\d)/, "$1/$2");
			value.birth = maskDate;
			this.setState({ value }, () => {
				this.props.onChange(value, path);
			});

			maskDate = maskDate.replace(/^(\d{2}.{1}\d{2})(\d)/, "$1/$2");
			value.birth = maskDate;
			this.setState({ value }, () => {
				this.props.onChange(value, path);
			});
