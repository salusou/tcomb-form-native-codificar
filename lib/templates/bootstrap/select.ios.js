...
class CollapsiblePickerIOS extends React.Component {
  ...
  render() {
    ...
    if (locals.value == "")
      if (locals.options.length > 0) {
        locals.value = locals.options[0].value;
      }

    var selectedOption = locals.options.find(
      option => option.value === locals.value
    );

    if (!selectedOption && locals.options.length > 0) {
      selectedOption = locals.options[0];
    }
