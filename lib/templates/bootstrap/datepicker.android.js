...
function datepicker(locals) {
  ...
  if (locals.config) {
    if (locals.config.format) {
      ...
      if(formattedValue == "Invalid Date" || formattedValue == "Invalid date"|| formattedValue == "invalid date" || !formattedValue) {
        formattedValue = locals.config.defaultValueText;
      }
    } else if (formattedValue == "Invalid Date" || formattedValue == "Invalid date"|| formattedValue == "invalid date" || !formattedValue) {
      formattedValue = locals.config.defaultValueText;
    }
  ...
  var value = locals.value ? (
    <Text style={[dateValueStyle, { color: "#3e3e3e", fontSize: 16}]}>{formattedValue}</Text>
  ) : null;
